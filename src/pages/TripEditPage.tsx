...
export function TripEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getTrip(tripId), getUserById(user.id)])
      .then(([t, u]) => {
        if (cancelled) return
        const own = u.trips.some((x) => x.id === tripId)
        setAllowed(own)
        if (own) {
          setInitial({
            title: t.title,
  ...
  if (!loading && !error && !allowed && initial === null) {
    return (
      <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900">
        ...
        <Link to={`/trips/${tripId}`} className="mt-2 inline-block font-medium underline">
          View trip
        </Link>
      </div>
    )
